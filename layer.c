...
int eventLayerMenu(XEvent ev,Window canvas){
  ...
  case ButtonPress:
	...
	if(write_state[selected_layer]!=0){//書き込み設定の確認
	  execFunc(ev,layer,layer_mask,selected_layer);//描画関数の実行
	  XCopyArea( dis, layer[selected_layer], layer_expose[selected_layer],
    ...
    if( ev.xbutton.button == 2 ){
      back_num = backHistory(layer,layer_mask); // 元に戻る
      if(back_num!=-1){
	XCopyArea( dis, layer[back_num], layer_expose[back_num],
		   gc, 0, 0, CANV_WIDTH , CANV_HEIGHT, 0, 0 );
      }
      return(0);
    }
