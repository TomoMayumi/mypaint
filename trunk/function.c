...
Window* initFuncMenu(Display *disp,Window root){
  ...
  //メニューウィンドウの作成
  /***************************/
  function = XCreateSimpleWindow( dis, root,
				  0, 0,
				  FUNC_WIDTH, FUNC_HEIGHT,
				  0, 0, 0x999999 );
  XSelectInput( dis, function, 0);
 
  /***************************/
  // GCの初期設定
  /***************************/
    
  func_gc = XCreateGC( dis, DefaultRootWindow(dis), 0, 0 );    // GCの標準設定
  XSetForeground( dis, func_gc, current_color  );    // 図形の色を設定
  XSetBackground( dis, func_gc, background);
  XSetLineAttributes( dis, func_gc, current_pen, current_line, current_cap, current_join);
...
void remapFuncMenu(){
  ...
  XSetLineAttributes(dis, gc, 0, LineSolid, CapRound, JoinMiter);
  switch(state){
  case SQUARE:
...
// 自由曲線
/**********************************/
void freeHand(XEvent ev,Layer *layer){
  int x,y;
  int i;
