...
int eventFuncMenu(XEvent ev){
  ...
  case ButtonPress:
    ...
    if ( ev.xany.window == colors_win ){   /* 色を指定して変更 */
      current_color = callColorSelect(dis, current_color);
      XSetForeground(dis, gc, current_color);
...
void line(XEvent ev){
  ...
  XCopyArea( dis, canvas, copy_pix[0], gc, 0, 0, CANV_WIDTH, CANV_HEIGHT, 0, 0 );
  ...
  while( !(ev.type == ButtonRelease && ev.xbutton.button == 1) ){
    if(ev.type == MotionNotify){
      ...
      XCopyArea( dis, copy_pix[1], canvas, gc, 0, 0, CANV_WIDTH, CANV_HEIGHT, 0, 0 );
      XFlush(dis);
    }
    XNextEvent( dis, &ev );
  }
...
void fill(XEvent ev){
  ...
  XCopyArea( dis, copy_pix[0], canvas, gc, 0, 0, CANV_WIDTH, CANV_HEIGHT, 0, 0 );
